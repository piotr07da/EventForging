...
public class given_aggregate_in_database : IAsyncLifetime
{
    ...
    [Fact]
    public async Task when_Get_and_call_some_operation_and_Save_with_None_expected_version_then_exception_thrown()
    {
        ...
        Assert.Equal(2, ex.RetrievedVersion);
        Assert.Null(ex.ActualVersion);
    }

    [Fact]
    public async Task when_Get_and_call_some_operation_and_Save_with_expected_version_different_than_retrieved_aggregate_version_then_exception_thrown()
    {
        var a = await _repository.GetAsync(_aggregateId);
    ...
    [Fact]
    public async Task when_Get_twice_and_call_some_operation_twice_and_Save_twice_with_Retrieved_expected_version_then_exception_thrown_at_second_Save()
    {
        ...
        await _repository.SaveAsync(_aggregateId, a1, ExpectedVersion.Retrieved, Guid.Empty, Guid.Empty);

        var ex = await Assert.ThrowsAnyAsync<EventForgingUnexpectedVersionException>(async () =>
        {
            await _repository.SaveAsync(_aggregateId, a2, ExpectedVersion.Retrieved, Guid.Empty, Guid.Empty);
        });
        Assert.Equal(ExpectedVersion.Retrieved, ex.ExpectedVersion);
        Assert.Equal(2, ex.RetrievedVersion);
        Assert.Equal((AggregateVersion)3, ex.ActualVersion);
    }

    [Fact]
    public async Task when_Get_twice_and_call_some_operation_twice_and_Save_twice_with_NOT_expected_version_then_exception_thrown_at_second_Save()
    {
        var a1 = await _repository.GetAsync(_aggregateId);
        var a2 = await _repository.GetAsync(_aggregateId);
    ...
    [Fact]
    public async Task when_getting_not_existing_aggregate_then_exception_thrown()
    {
